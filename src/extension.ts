...
class BlobifyHoverProvider implements vscode.HoverProvider {
  ...
  private getSwitchInfo(switchName: string): string | null {
    const switches: { [key: string]: string } = {
      ...
      "list-ignored": "**List Ignored Patterns**\n\nList built-in ignored patterns and exit\n\n`@list-ignored`",
...
class BlobifyCompletionProvider implements vscode.CompletionItemProvider {
  ...
  private getSwitchCompletions(): vscode.CompletionItem[] {
    const items = [
      ...
      { name: "debug", detail: "Enable debug output", priority: 95 },
      { name: "noclean", detail: "Disable scrubadub processing", priority: 90 },
      { name: "no-line-numbers", detail: "Disable line numbers", priority: 85 },
      { name: "no-index", detail: "Disable file index section", priority: 80 },
      { name: "no-content", detail: "Exclude file contents", priority: 75 },
      { name: "no-metadata", detail: "Exclude file metadata", priority: 70 },
      { name: "suppress-excluded", detail: "Suppress excluded files", priority: 65 },
      { name: "list-ignored", detail: "List built-in ignored patterns", priority: 60 },
      { name: "output", detail: "Output file path", insertText: "output=${1:filename.txt}", priority: 55 },
      {
      ...
      { name: "filter (functions)", detail: "Python functions", insertText: "filter=functions:^\\s*(def\\s+\\w+.*:|async\\s+def\\s+\\w+.*:)", priority: 45 },
      { name: "filter (classes)", detail: "Python classes", insertText: "filter=classes:^\\s*class\\s+\\w+.*:", priority: 40 },
      { name: "filter (imports)", detail: "Python imports", insertText: "filter=imports:^\\s*(from\\s+\\w+.*import.*|import\\s+\\w+.*)", priority: 35 },
